...
DOTNETFLAGS=--nodoreuse:false -v n
...
#
	...
	# remove offscreen canvas for single-threaded mode
	$(SED) -i 's/var offscreenCanvases \?= \?{};/var offscreenCanvases={};/' frontend/public/_framework/dotnet.native.*.js
	# dotnet messed up
	$(SED) -i 's/this.appendULeb(32768)/this.appendULeb(65535)/' frontend/public/_framework/dotnet.runtime.*.js
	# use runEmAsmFunction for single-threaded mode
	$(SED) -i 's/return runMainThreadEmAsm(code, sigPtr, argbuf, 1);/return runEmAsmFunction(code, sigPtr, argbuf);/' frontend/public/_framework/dotnet.native.*.js
serve: build
